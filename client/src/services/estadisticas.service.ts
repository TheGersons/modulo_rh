...
export const estadisticasService = {
  ...
  getByArea: async (areaId: string) => {
    const response = await apiClient.get(API_ENDPOINTS.estadisticasArea(areaId));
    return response.data;
  },

  getByEmpleado: async (empleadoId: string) => {
    const response = await apiClient.get(API_ENDPOINTS.estadisticasEmpleado(empleadoId));
    return response.data;
  },

  getTendencias: async (meses: number = 6) => {
    const response = await apiClient.get(`${API_ENDPOINTS.estadisticasTendencias}?meses=${meses}`);
    return response.data;
  },
