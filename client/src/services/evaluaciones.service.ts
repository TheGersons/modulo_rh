...
export const evaluacionesService = {
  ...
  getByEmpleado: async (empleadoId: string) => {
    const response = await apiClient.get(API_ENDPOINTS.evaluacionesByEmpleado(empleadoId));
    return response.data;
  },

  getByArea: async (areaId: string) => {
    const response = await apiClient.get(API_ENDPOINTS.evaluacionesByArea(areaId));
    return response.data;
  },
  ...
  update: async (id: string, data: any) => {
    const response = await apiClient.patch(API_ENDPOINTS.evaluacionById(id), data);
    return response.data;
  },
