...
import {
    Target, AlertTriangle, CheckCircle, Clock, Building, Briefcase,
    TrendingUp, Eye, ExternalLink, X, ClipboardList, Activity,
} from 'lucide-react';
...
export interface EmpleadoKpisData {
...
export interface EmpleadoOrdenesData {
    empleado: { id: string; nombre: string; apellido: string };
...
export const NIVEL_ALERTA: Record<string, { bg: string; text: string; dot: string }> = {
...
export const KPI_ESTADO: Record<string, { bg: string; text: string; label: string; border: string }> = {
...
export function estaVencida(fechaLimite: string) {
    return new Date(fechaLimite) < new Date();
}

// ─── Modal de perfil ──────────────────────────────────────────────────────────

export default function ModalPerfilEmpleado({ empleado, periodo, onClose, navigate }: {
