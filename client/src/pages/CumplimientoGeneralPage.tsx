...
import {
    Users, Target, AlertTriangle, CheckCircle, Clock, Building, Briefcase,
    XCircle, Eye, AlertCircle, RefreshCw, Filter, FileCheck, ExternalLink,
    X, BarChart2, ClipboardList, Activity, TrendingUp, Search, Download,
    ArrowUpDown, Globe,
} from 'lucide-react';
...
function colorPct(pct: number): { text: string; bg: string; bar: string } {
    if (pct >= 80) return { text: 'text-green-600', bg: 'bg-green-50', bar: 'bg-green-500' };
    if (pct >= 50) return { text: 'text-yellow-600', bg: 'bg-yellow-50', bar: 'bg-yellow-500' };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0 ${colorKpi.bar}`}>
                    ...
                    {tab === 'resumen' && (
                        <>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    { label: 'Cumplimiento KPIs', value: `${pct}%`, color: colorKpi.text, bg: colorKpi.bg },
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <div className="flex items-center gap-4 text-xs text-gray-500 flex-wrap">
                                                                {kpi.meta !== undefined && kpi.meta !== null && (
                                                                    <span>Meta: <span className="font-medium text-gray-700">{op} {kpi.meta} {kpi.unidad}</span></span>
                                                                )}
...
type SortKey = 'nombre' | 'area' | 'cumplimiento' | 'rechazados' | 'vencidas';

export default function CumplimientoGeneralPage() {
    ...
    const [filtroCumplimiento, setFiltroCumplimiento] = useState<'todos' | 'bajo' | 'medio' | 'alto'>('todos');
    const [filtroProblemas, setFiltroProblemas] = useState(false);
    const [sortKey, setSortKey] = useState<SortKey>('cumplimiento');
    const [sortAsc, setSortAsc] = useState(true);

    useEffect(() => { cargarDatos(); /* eslint-disable-next-line */ }, [periodo]);
    ...
    const cargarDatos = async () => {
        try {
            ...
            setError(err.response?.data?.message || 'Error al cargar el cumplimiento');
    ...
    const empleadosFiltrados = useMemo(() => {
        ...
        let lista = data.empleados.filter((emp) => {
            const matchBusqueda = !term ||
                `${emp.nombre} ${emp.apellido}`.toLowerCase().includes(term) ||
                emp.area?.nombre.toLowerCase().includes(term) ||
                emp.puesto?.nombre.toLowerCase().includes(term);
            const matchArea = filtroArea === 'todas' || emp.area?.id === filtroArea;
            const pct = emp.kpis.porcentajeCumplimiento;
            const matchCumpl =
                filtroCumplimiento === 'todos' ||
                (filtroCumplimiento === 'alto' && pct >= 80) ||
                (filtroCumplimiento === 'medio' && pct >= 50 && pct < 80) ||
                (filtroCumplimiento === 'bajo' && pct < 50);
            const matchProblemas = !filtroProblemas || emp.tieneProblemas;
            return matchBusqueda && matchArea && matchCumpl && matchProblemas;
        });
        ...
        lista = [...lista].sort((a, b) => {
            ...
            switch (sortKey) {
                case 'nombre':
                    cmp = `${a.apellido} ${a.nombre}`.localeCompare(`${b.apellido} ${b.nombre}`);
                    break;
                case 'area':
                    cmp = (a.area?.nombre ?? '').localeCompare(b.area?.nombre ?? '');
                    break;
                case 'cumplimiento':
                    cmp = a.kpis.porcentajeCumplimiento - b.kpis.porcentajeCumplimiento;
                    break;
                case 'rechazados':
                    cmp = a.kpis.rechazados - b.kpis.rechazados;
                    break;
                case 'vencidas':
                    cmp = a.ordenes.vencidas - b.ordenes.vencidas;
                    break;
            }
        ...
        return lista;
    }, [data, busqueda, filtroArea, filtroCumplimiento, filtroProblemas, sortKey, sortAsc]);

    const cumplimientoPromedio = useMemo(() => {
        if (!data || data.empleados.length === 0) return 0;
        const con = data.empleados.filter((e) => e.kpis.total > 0);
        if (con.length === 0) return 0;
        const sum = con.reduce((acc, e) => acc + e.kpis.porcentajeCumplimiento, 0);
        return Math.round(sum / con.length);
    }, [data]);

    const distribucion = useMemo(() => {
        if (!data) return { alto: 0, medio: 0, bajo: 0, sinKpis: 0 };
        let alto = 0, medio = 0, bajo = 0, sinKpis = 0;
        data.empleados.forEach((e) => {
            if (e.kpis.total === 0) sinKpis++;
            else if (e.kpis.porcentajeCumplimiento >= 80) alto++;
            else if (e.kpis.porcentajeCumplimiento >= 50) medio++;
            else bajo++;
        });
        return { alto, medio, bajo, sinKpis };
    }, [data]);

    const exportarCSV = () => {
        if (!data) return;
        const headers = ['Empleado', 'Área', 'Puesto', 'Cumplimiento KPIs (%)', 'KPIs Total', 'KPIs Aprobados', 'KPIs Rechazados', 'KPIs En Revisión', 'KPIs Pendientes', 'Órdenes Activas', 'Órdenes Vencidas', 'Alertas', 'Evidencias por Revisar'];
        const filas = empleadosFiltrados.map((e) => [
            `${e.nombre} ${e.apellido}`,
            e.area?.nombre ?? '',
            e.puesto?.nombre ?? '',
            e.kpis.porcentajeCumplimiento,
            e.kpis.total,
            e.kpis.aprobados,
            e.kpis.rechazados,
            e.kpis.enRevision,
        ...
        a.href = url;
        a.download = `cumplimiento-general-${periodo}.csv`;
        a.click();
        URL.revokeObjectURL(url);
    };

    const cambiarSort = (key: SortKey) => {
        if (sortKey === key) setSortAsc(!sortAsc);
        else { setSortKey(key); setSortAsc(key === 'nombre' || key === 'area'); }
    };
        ...
        <Layout>
            <div className="flex items-center justify-center h-screen">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-gray-600">Cargando cumplimiento general...</p>
                </div>
            </div>
        </Layout>
    );
        ...
        <Layout>
            <div className="p-8">
                <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-gray-200">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Sin empleados</h3>
                    <p className="text-gray-500 text-sm">No hay empleados registrados en la empresa.</p>
                </div>
            </div>
        </Layout>
    );

    const { resumen, areas } = data;
    const colorProm = colorPct(cumplimientoPromedio);
            ...
            <div className="p-8 space-y-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Cumplimiento General</h1>
                            <p className="text-gray-500 mt-1 text-sm">Vista global de todos los empleados · Período <span className="font-medium text-gray-700">{periodo}</span></p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={cargarDatos} className="p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors" title="Recargar">
                            <RefreshCw className="w-5 h-5" />
                        </button>
                    </div>
                </div>

                {/* Tarjeta destacada: cumplimiento promedio */}
                <div className={`rounded-xl p-6 shadow-sm border-2 ${colorProm.bg} ${colorProm.text.replace('text-', 'border-').replace('-600', '-200')}`}>
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div>
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Cumplimiento promedio de la empresa</p>
                            <div className="flex items-baseline gap-3">
                                <p className={`text-5xl font-bold ${colorProm.text}`}>{cumplimientoPromedio}%</p>
                                <p className="text-sm text-gray-500">de {data.empleados.filter((e) => e.kpis.total > 0).length} empleados con KPIs</p>
                            </div>
                        </div>
                        <div className="flex gap-3">
                            <div className="bg-green-50 rounded-lg p-3 text-center border border-green-200 min-w-[90px]">
                                <p className="text-xl font-bold text-green-700">{distribucion.alto}</p>
                                <p className="text-xs text-green-600">Alto (≥80%)</p>
                            </div>
                            <div className="bg-yellow-50 rounded-lg p-3 text-center border border-yellow-200 min-w-[90px]">
                                <p className="text-xl font-bold text-yellow-700">{distribucion.medio}</p>
                                <p className="text-xs text-yellow-600">Medio (50-79%)</p>
                            </div>
                            <div className="bg-red-50 rounded-lg p-3 text-center border border-red-200 min-w-[90px]">
                                <p className="text-xl font-bold text-red-700">{distribucion.bajo}</p>
                                <p className="text-xs text-red-600">Bajo (&lt;50%)</p>
                            </div>
                            {distribucion.sinKpis > 0 && (
                                <div className="bg-gray-50 rounded-lg p-3 text-center border border-gray-200 min-w-[90px]">
                                    <p className="text-xl font-bold text-gray-600">{distribucion.sinKpis}</p>
                                    <p className="text-xs text-gray-500">Sin KPIs</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 flex items-center gap-3 flex-wrap">
                    ...
                    <select value={filtroCumplimiento} onChange={(e) => setFiltroCumplimiento(e.target.value as any)}
                        className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                    ...
                    <span className="text-xs text-gray-400 ml-auto">{empleadosFiltrados.length} de {data.empleados.length}</span>
                </div>
                            ...
                            <table className="w-full">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                            <button onClick={() => cambiarSort('nombre')} className="flex items-center gap-1 hover:text-gray-900">
                                                Empleado <ArrowUpDown className="w-3 h-3" />
                                            </button>
                                        </th>
                                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                            <button onClick={() => cambiarSort('area')} className="flex items-center gap-1 hover:text-gray-900">
                                                Área / Puesto <ArrowUpDown className="w-3 h-3" />
                                            </button>
                                        </th>
                                        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider min-w-[200px]">
                                            <button onClick={() => cambiarSort('cumplimiento')} className="flex items-center gap-1 hover:text-gray-900">
                                                Cumplimiento KPIs <ArrowUpDown className="w-3 h-3" />
                                            </button>
                                        </th>
                                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                            <button onClick={() => cambiarSort('rechazados')} className="flex items-center gap-1 mx-auto hover:text-gray-900">
                                                KPIs Rojos <ArrowUpDown className="w-3 h-3" />
                                            </button>
                                        </th>
                                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">Órdenes</th>
                                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                            <button onClick={() => cambiarSort('vencidas')} className="flex items-center gap-1 mx-auto hover:text-gray-900">
                                                Vencidas <ArrowUpDown className="w-3 h-3" />
                                            </button>
                                        </th>
                                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">Alertas</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {empleadosFiltrados.map((emp) => {
                                        ...
                                        const sinKpis = emp.kpis.total === 0;
                                        return (
                                            <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0 ${sinKpis ? 'bg-gray-400' : c.bar}`}>
                                                            {emp.nombre[0]}{emp.apellido[0]}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <p className="text-sm font-medium text-gray-900 truncate">{emp.nombre} {emp.apellido}</p>
                                                            <p className="text-xs text-gray-500 capitalize">{emp.role}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3">
                                                    <p className="text-xs text-gray-700 truncate">{emp.area?.nombre ?? '—'}</p>
                                                    <p className="text-xs text-gray-400 truncate">{emp.puesto?.nombre ?? '—'}</p>
                                                </td>
                                                <td className="px-4 py-3">
                                                    {sinKpis ? (
                                                        <span className="text-xs text-gray-400 italic">Sin KPIs asignados</span>
                                                    ) : (
                                                        <div className="space-y-1">
                                                            <div className="flex items-center gap-2">
                                                                <span className={`text-sm font-semibold ${c.text} min-w-[42px]`}>{pct}%</span>
                                                                <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                                                                    <div className={`h-2 rounded-full transition-all ${c.bar}`} style={{ width: `${pct}%` }} />
                                                                </div>
                                                            </div>
                                                            <p className="text-xs text-gray-400">{emp.kpis.aprobados}/{emp.kpis.total} aprobados</p>
                                                        </div>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3 text-center">
                                                    {emp.kpis.rechazados > 0 ? (
                                                        <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-medium">{emp.kpis.rechazados}</span>
                                                    ) : (
                                                        <span className="text-xs text-gray-400">0</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3 text-center">
                                                    <span className="text-xs text-gray-600">{emp.ordenes.activas}</span>
                                                </td>
                                                <td className="px-4 py-3 text-center">
                                                    {emp.ordenes.vencidas > 0 ? (
                                                        <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded-full text-xs font-medium">{emp.ordenes.vencidas}</span>
                                                    ) : (
                                                        <span className="text-xs text-gray-400">0</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3 text-center">
                                                    {emp.alertas.length > 0 ? (
                                                        <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">{emp.alertas.length}</span>
                                                    ) : (
                                                        <span className="text-xs text-gray-400">0</span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3 text-right">
                                                    <button onClick={() => setEmpleadoModal(emp)}
                                                        ...
                                                        <BarChart2 className="w-3.5 h-3.5" />Ver
                                                    </button>
                                                </td>
                                            </tr>
                                        );
